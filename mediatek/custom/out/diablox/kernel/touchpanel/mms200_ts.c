...
#define MELFAS_TURLY_H2_FW_VERSION 	0xf7
#define MELFAS_TURLY_H3_FW_VERSION 	0x09
#define MELFAS_BIEL_FW_VERSION 		0x05
...
*/
#if 1
const u8 MELFAS_biel_binary[] = {
#include "MCH_TDIABLOX_B_R01_V05.mfsb.h"
};
const u8 MELFAS_turly_h2_binary[] = {
#include "MCH_TDIABLOX_T_R02_VF7.mfsb.h"
};
const u8 MELFAS_turly_h3_binary[] = {
#include "MCH_TDIABLOX_T_R03_V09.mfsb.h"
};
...
static int melfas_touch_event_handler(void *unused)
{
	...
	do {    	
		...
		read_num = buf[0];
		//TPD_DMESG("melfas_touch_event_handler,read_num = %d,interrupt_count = %d \n",read_num,interrupt_count);
				...
				Touch_Type = (buf[i] >> 5) & 0x03;
					...
					if (!touchState && !g_Mtouch_info[fingerID].pressed) {
						...
						continue;
                    			...
                    			g_Mtouch_info[fingerID].pos_x = (uint16_t)(buf[i + 2] | ((buf[i + 1] & 0xf) << 8));
                    			g_Mtouch_info[fingerID].pos_y = (uint16_t)(buf[i + 3] | (((buf[i + 1] >> 4 ) & 0xf) << 8));
...
int melfas_check_firmware(struct i2c_client *client)
{
    ...
    if (ret >= 0)
    {
        ...
        if (tp_factory_id_cmd == TS_TYPE_TRULY) {
        	...
        	if (val[0] < MELFAS_BIEL_FW_VERSION)
            		ret = 1;
        	else
            		ret = 0;
        }
...
struct firmware fw_info_biel = 
{
	.size = sizeof(MELFAS_biel_binary),
	.data = &MELFAS_biel_binary[0],
};
	...
	if (ret != 0) {
		...
		if (tp_factory_id_cmd == TS_TYPE_TRULY) {
    			if (hardware_ver == 1) {
        			...
        			TPD_DMESG("[melfas_tpd] MELFAS_bin = %x, addr = %x\n", 
			...
			TPD_DMESG("[melfas_tpd] MELFAS_bin_len = %x\n", 
				fw_info_biel.size);
        		TPD_DMESG("[melfas_tpd] MELFAS_bin = %x, addr = %x\n", 
        			fw_info_biel.data[0],fw_info_biel.data);
        		mms_fw_update_controller(&fw_info_biel,client);	
		}
#endif
    }
    return ret;
