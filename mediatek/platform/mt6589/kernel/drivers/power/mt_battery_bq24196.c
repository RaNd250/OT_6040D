...
void tbl_charger_otg_vbus(int mode)
{
    ...
    if(mode&0xFF)
    {
    	mt_set_gpio_mode(GPIO79,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO79,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO79,GPIO_OUT_ZERO);     
		
		mt_set_gpio_mode(GPIO145,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO145,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO145,GPIO_OUT_ONE);     

	 	mt_set_gpio_mode(GPIO174,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO174,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO174,GPIO_OUT_ONE);    
        udelay(1600);	
    ...
    {
		mt_set_gpio_mode(GPIO79,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO79,GPIO_DIR_IN);

		mt_set_gpio_mode(GPIO174,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO174,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO174,GPIO_OUT_ZERO);  


		mt_set_gpio_mode(GPIO145,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO145,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO145,GPIO_OUT_ZERO);   
	
	    bq24196_set_chg_config(0x1); //OTG & Charge enabled
...
static int chargingCurrentStep = 1024; 
...
void select_charging_curret_bq24196(void)
{
    ...
    else
    {
        ...
        {
           // g_temp_CC_value = AC_CHARGER_CURRENT;
            g_temp_CC_value =chargingCurrentStep;
            bq24196_set_iinlim(0x7); //IN current limit at 2A			
            bq24196_set_ac_current();
        }
...
void ChargerHwInit_bq24196(void)
{
    ...
    //pull CE low
    mt_set_gpio_mode(GPIO22,GPIO_MODE_GPIO);  
    mt_set_gpio_dir(GPIO22,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO22,GPIO_OUT_ZERO);       
    bq24196_set_en_hiz(0x0);
    bq24196_set_vindpm(0x0); //VIN DPM check 4.68V
...
//    if(upmu_get_cid() == 0x1020)
	...
	bq24196_set_iterm(0x1); //Termination current 128mA
...
void BAT_thread_bq24196(void)
{
	...
	if( (CHR_Type_num==CHARGING_HOST) || (CHR_Type_num==STANDARD_CHARGER) )
	  	{
	  	 printk("bq24196 chargingCurrentStep %d,%d\n",chargingCurrentStep,g_temp_CC_value);
		bq24196_read_interface(0x8, &ret_val, 0x1, 0x3);
	  	 if(!ret_val)
	  	 	{
	  	 	 if(chargingCurrentStep <= 2484)
	  	 	     {
	  	 	      if(chargingCurrentStep >= 1536)
	  	 	     	   {
	  	 	     	  		chargingCurrentStep +=256;
	  	 	     	   }
			      else
			         {
                             chargingCurrentStep += 128;

			     	  }
	  	 	     }
	  	 	}
		
              printk("bq24196 chargingCurrentStep1 %d,%d,ret_val=%d\n",chargingCurrentStep,g_temp_CC_value,ret_val);
	      }
...
//			if(upmu_get_cid() == 0x1020)
      if(0)
				...
				bq24196_set_sys_min(0x5); //Minimum system voltage 3.5V             
            bq24196_set_iprechg(0x1); //Precharge current 256mA
            bq24196_set_iterm(0x1); //Termination current 256mA
            bq24196_set_vreg(0x2C); //VREG 4.208V
            bq24196_set_batlowv(0x1); //BATLOWV 3.0V
            bq24196_set_vrechg(0x0); //VRECHG 0.1V (4.108V)
            bq24196_set_en_term(0x1); //Enable termination
