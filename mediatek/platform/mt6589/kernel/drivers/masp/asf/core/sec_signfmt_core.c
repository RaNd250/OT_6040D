    ...
    char *hash_buf, unsigned int hash_len, char *ext_buf)
{
    ...
    if (is_signfmt_v3(&img_hdr) && (SECCFG_V3 == get_seccfg_ver()))
    {
        ...
        ext_hdr_len = img_if->v3.ext_len - shdr_sig_len(&img_if->v3.header);
    ...
    if(SECCFG_V3 == get_seccfg_ver())
    {
        ...
        mcpy(verify_buf+SEC_IMG_HEADER_SIZE+file_content_len, 
            (char*)ext_buf+img_if->v3.ext_offset+shdr_sig_len(&img_if->v3.header), ext_hdr_len);
    }
    else
    {
...
int sec_signfmt_calculate_image_hash(char* part_name, SEC_IMG_U *img_if, char *hash_buf, unsigned int hash_len, char *ext_buf)
{
    ...
    if (SECCFG_V3 == get_seccfg_ver() && is_signfmt_v3((SEC_IMG_HEADER*)&img_if->v3.header.va))
    {
