    ...
    )
{
    ...
    IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
        ...
        case ELEM_ID_DS_PARAM_SET:
            ...
            break;
#if CFG_SUPPORT_DFS
		case ELEM_ID_CH_SW_ANNOUNCEMENT://channel switch announcement
			rlmProcessChannelSwitchIE(prAdapter, (P_IE_CHNL_SWITCH_T)pucIE);
		  	break;
#endif
        default:
            break;
        ...
        )
{
    ...
    if (prRxFrame->ucAction == ACTION_CHNL_SWITCH) {
        rlmProcessChannelSwitchIE(prAdapter, (P_IE_CHNL_SWITCH_T)prRxFrame->aucInfoElem);
    }
...
rlmProcessChannelSwitchIE(
        P_ADAPTER_T prAdapter,
        P_IE_CHNL_SWITCH_T  prChannelSwitchIE
        )
{
    ...
    printk("[5G DFS] ucSwitchMode[%d], ucSwitchCount[%d], ucNewChannel[%d] \r\n", \
        prChannelSwitchIE->ucSwitchMode,	\
        prChannelSwitchIE->ucSwitchCount,	\
        prChannelSwitchIE->ucNewChannel);
	if(prChannelSwitchIE->ucSwitchMode == 1){
		prAisBssInfo = &(prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX]);
		printk("[5G DFS] switch channel [%d]->[%d] \r\n", prAisBssInfo->ucPrimaryChannel, prChannelSwitchIE->ucNewChannel);
		prAisBssInfo->ucPrimaryChannel = prChannelSwitchIE->ucNewChannel;
