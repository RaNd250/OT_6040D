	...
	typedef enum engineer_index
{
    ...
    extern OV5650_OP_TYPE OV5650_g_iOV5650_Mode;
...
//    extern SensorInfo OV5650_g_CCT_MainSensor;
  //  extern kal_uint8 OV5650_g_CCT_FirstGrabColor;
	...
	#define OV5650_VIDEO_NORMALMODE_FRAME_RATE		(25)
	#define OV5650_VIDEO_NIGHTMODE_FRAME_RATE		(13)
    ...
    #define OV5650_QXGA_MODE_WITHOUT_DUMMY_PIXELS  (3112)  // QXGA mode's pixel # in one HSYNC w/o dummy pixels
    #define OV5650_XGA_MODE_WITHOUT_DUMMY_PIXELS   (OV5650_QXGA_MODE_WITHOUT_DUMMY_PIXELS / 2)    // XGA mode's pixel # in one HSYNC w/o dummy pixels
    #define OV5650_PV_PERIOD_PIXEL_NUMS    (OV5650_XGA_MODE_WITHOUT_DUMMY_PIXELS) // dummy pixels is down sampled by half in XGA mode
    ...
    #define OV5650_QXGA_MODE_WITHOUT_DUMMY_LINES   (1968)//(1568)  // QXGA mode's HSYNC # in one HSYNC w/o dummy lines
    #define OV5650_XGA_MODE_WITHOUT_DUMMY_LINES    (984)//(784)   // XGA mode's HSYNC # in one HSYNC w/o dummy lines
    #define OV5650_PV_PERIOD_LINE_NUMS     (OV5650_XGA_MODE_WITHOUT_DUMMY_LINES)  // XGA mode's HSYNC # in one VSYNC period

    // SENSOR EXPOSURE LINE LIMITATION
    #define OV5650_FULL_MAX_LINES_PER_FRAME    (1944)  // QXGA mode 1568
    #define OV5650_FULL_EXPOSURE_LIMITATION    (OV5650_FULL_MAX_LINES_PER_FRAME)
    #define OV5650_PV_MAX_LINES_PER_FRAME  (OV5650_XGA_MODE_WITHOUT_DUMMY_LINES)  // # of lines in one XGA frame
    #define OV5650_PV_EXPOSURE_LIMITATION  (OV5650_PV_MAX_LINES_PER_FRAME)

    // sensor's full resolution
    #define OV5650_IMAGE_SENSOR_FULL_WIDTH     (2592) // (2584)
    #define OV5650_IMAGE_SENSOR_FULL_HEIGHT    (1944) //(1944)

    // sensor's full resolution
    #define OV5650_IMAGE_SENSOR_FULL_WIDTH_DRV     (2592 - 2 - (OV5650_ISP_INTERPOLATIO_FILTER_WIDTH - 1)) 
    #define OV5650_IMAGE_SENSOR_FULL_HEIGHT_DRV    (1944 - 2 - (OV5650_ISP_INTERPOLATIO_FILTER_HEIGHT - 1)) 

    // resolution for preview
    #define OV5650_IMAGE_SENSOR_PV_WIDTH   (1280)//(1024)//640
    #define OV5650_IMAGE_SENSOR_PV_HEIGHT  (960)//(768)//480
    
//   #define ISP_INTERPOLATIO_FILTER_WIDTH   (7)
//   #define ISP_INTERPOLATIO_FILTER_HEIGHT  (7)
        #define OV5650_ISP_INTERPOLATIO_FILTER_WIDTH   (15)//5//11
        #define OV5650_ISP_INTERPOLATIO_FILTER_HEIGHT  (11)//7//14

    /* SENSOR READ/WRITE ID */
    #define OV5650_WRITE_ID (0x6C)
    #define OV5650_READ_ID  (0x6D)

    /* SENSOR CHIP VERSION */
//    #define OV5650_SENSOR_ID    (0x3641)    // rev.2A
    //#define OV5650_SENSOR_ID    0x364C    // rev.2C


//added by mandrave
#define OV5650_USE_OTP

#if defined(OV5650_USE_OTP)

struct ov5650_otp_struct
{
...
//format index
#define OV5650_PV_FORMAT_INDEX   1 //1280x960 mode
#define OV5650_FULL_FORMAT_INDEX 2 //Full resolution mode
...
UINT32 OV5650Control(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData);
UINT32 OV5650FeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara,UINT32 *pFeatureParaLen);
UINT32 OV5650GetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT *pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData);
