...
static int cal_params[4] = {0};//max, mean, high, loww
static int cal_data[10]={0}; //cal data

#endif 
/******************************************************************************
 * power on calirbation functions
*******************************************************************************/
#define POWER_ON_CALI

static void cm36283_prox_calibration(struct i2c_client *client);
int cm36283_write_ps_thd_hig(struct i2c_client *client, u8 thd);
int cm36283_write_ps_thd_low(struct i2c_client *client, u8 thd);
...
//john0x03
			databuf[2] = 0x43;//databuf[1];
			databuf[1] = 0xC2; 
			databuf[0]= CM36283_REG_PS_CONF1_2;
#if JOHN_DEBUG
			APS_LOG("john_enable_ps CM36283_REG_PS_CONF1_2's databuf[1] = %x\n",databuf[1]);
#endif 
...
#if  JOHN_DEBUG
        int i;
        printk("john debug start --------------------------------------------\n");	

        printk("john prox_threshold_hi =%d\n",cal_params[0]);	
        printk("john prox_threshold_lo = %d\n",cal_params[1]);	
        printk("john prox_mean =%d\n",cal_params[2]);	
        printk("john prox_max   =%d\n", cal_params[3]);\	
        printk("john cm36283_get_ps_value's  ps value = %d\n",ps);  

        for(i=0;i<10;i++)  
        {     
            printk("john cal_data[%d]=%d\n",i,cal_data[i]);
        }   
        
        printk("john debug end --------------------------------------------\n");
#endif
...
static void cm36283_eint_work(struct work_struct *work)
{
	...
	printk("john_cm36283_eint_work start\n");
#if 1
	res = cm36283_check_intr(obj->client);
	if(res != 0){
		goto EXIT_INTR_ERR;
...
static void cm36283_prox_calibration(struct i2c_client *client)
{
	...
	//enable ps
	cm36283_enable_ps(client, 1);
	mdelay(60);

	prox_min = 255;
	for(i = 0; i < 10; i++)
	{
		if((err = cm36283_read_ps(client, &prox_data[i])))
		{
			APS_ERR("cm3623_prox_calibration fail: %d\n", i);
			return err;
		}
		...
		printk("john_XXXXXXX prox_data[%d] = %d XXXXXXX\n",i ,prox_data[i]);
        ...
        prox_mean = prox_sum / 10; //get the ave value.
        
	prox_threshold_hi = prox_mean+10;//((((prox_max - prox_mean) * 500) + 50) / 100) + prox_mean;//prox_mean*4;
	prox_threshold_lo = prox_mean+5;//((((prox_max - prox_mean) * 100) + 50) / 100) + prox_mean;
	


	if(prox_min > 128 || 0 == prox_min || prox_max == 255) 
        {   
	...
	//disable ps
#if JOHN_DEBUG
			cal_params[0] = prox_threshold_hi;     
            cal_params[1] = prox_threshold_lo;        
            cal_params[2] = prox_mean;       
            cal_params[3] = prox_max;
#endif  
	cm36283_enable_ps(client, 0);
	mdelay(60);
}
...
//#if JOHN_DEBUG
				APS_LOG("john_cm36283_unlocked_ioctl case ALSPS_GET_PS_DATA:\n");
//#endif
...
static int cm36283_init_client(struct i2c_client *client)
{
	...
	u8 databuf[3];    
	int res = 0;
	printk("john_debug xxxxxxxxxxxxxxxxxxxx 1");
	databuf[0] = CM36283_REG_ALS_CONF;
...
#if JOHN_DEBUG
	APS_LOG("john_init_client_CM36283_REG_PS_CONF1_2 databuf[1] =%x\n");
#endif
	//john0x03
	databuf[0] = CM36283_REG_PS_CONF1_2;
	databuf[1] = 0xC2;
...
#ifdef POWER_ON_CALI
         cm36283_prox_calibration(client);
#endif 
