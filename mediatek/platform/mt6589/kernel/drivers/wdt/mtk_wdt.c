...
//This functioon is to fix system reboot cannot reboot bug.
void platoform_avoid_reset_error(void){
		upmu_set_rg_sysrstb_en(0);		//disable reset sysrstb.
		upmu_set_rg_rst_part_sel(1);
...
void wdt_arch_reset(char mode)
{
	...
	spin_lock(&rgu_reg_operation_spinlock);
	/*disable pmic sysrst register, or phone can not reboot*/
	platoform_avoid_reset_error();
	/* Watchdog Rest */
	DRV_WriteReg32(MTK_WDT_RESTART, MTK_WDT_RESTART_KEY);
