...
#define FM_CQI_BUF_SIZE  16
fm_u8 soft_mute_tune_CQI[FM_CQI_BUF_SIZE];
...
static struct fm_i2s_info i2s_info = {
    .status = 0,    //i2s off
    .mode = 0,      //slave mode
...
extern int MT6620_RDS_Reset_Block_Counter(void);
...
static int  fm_soft_mute_tune(struct fm *fm, struct fm_softmute_tune_cqi_t *parm);
static int fm_is_dese_chan(struct fm *pfm, fm_u16 freq);

static int  fm_scan(struct fm *fm, struct fm_scan_parm *parm);
static int  fm_scan_new(struct fm *fm, struct fm_scan_t *para);
static int  fm_seek_new(struct fm *fm, struct fm_seek_t *para);
static int  fm_tune_new(struct fm *fm, struct fm_tune_t *para);
...
static int MT6620_Seek(uint16_t min_freq, uint16_t max_freq, uint16_t *pFreq, uint16_t seekdir, uint16_t space)
{
    ...
    //Clear "FLAG_SEEK" & "FLAG_SEEK_DONE" to make sure we do make a new seek
    spin_lock(&flag_lock);
    flag &= ~(FLAG_SEEK | FLAG_SEEK_DONE);
    spin_unlock(&flag_lock);
	...
	while(!(!(cnt < scan_cnt) || (freq > max_freq) || (freq < min_freq))){
		...
		if(scandir == SCAN_UP){
            if(freq == FMTX_SCAN_HOLE_LOW){
                freq += (FMTX_SCAN_HOLE_HIGH - FMTX_SCAN_HOLE_LOW + step);
            }else{
                freq += step;
            }
...
static int MT6620_ScanForceStop()
{
    ...
    spin_lock(&flag_lock);
    flag |= (FLAG_SCAN_DONE | FLAG_SEEK_DONE);
    stop_scan_flag = 1;
    spin_unlock(&flag_lock);
            ...
            case FM_TASK_RX_PARSER_PKT_PAYLOAD:
                switch(opcode){
                    case FM_SEEK_OPCODE:
                        ...
                        if((i+1) < RX_BUF_SIZE){
                            seek_result = (rx_buf[i] + (rx_buf[i+1] << 8));
                        }
					...
					case FM_SOFT_MUTE_TUNE_OPCODE:
					{
                        ...
                        if (length >= 2)
                        {
                        	memcpy(soft_mute_tune_CQI, &rx_buf[i], (length > FM_CQI_BUF_SIZE) ? FM_CQI_BUF_SIZE : length);
							/*FM_LOG_NTC(D_G1, "freq %d, 0x%04x, 0x%04x, 0x%04x, 0x%04x, 0x%04x, 0x%04x\n", 
								(rx_buf[i+3]<<8)|rx_buf[i+2],
								(rx_buf[i+5]<<8)|rx_buf[i+4],
								(rx_buf[i+7]<<8)|rx_buf[i+6],
								(rx_buf[i+9]<<8)|rx_buf[i+8],
								(rx_buf[i+11]<<8)|rx_buf[i+10],
								(rx_buf[i+13]<<8)|rx_buf[i+12],
								(rx_buf[i+15]<<8)|rx_buf[i+14]);*/
                        }
                        //FM_LOG_NTC(D_G0,"get soft mute tune event:length=%d\n", length);
                        wake_up(&fm_wq);
						break;
					}
                    case FM_SCAN_OPCODE:
...
//              return -EPERM;
		...
		{
            struct fm_softmute_tune_cqi_t parm;
            FM_LOG_NTC(D_IOCTL,"......FM_IOCTL_SOFT_MUTE_TUNE......\n");
            if (FALSE == fm->chipon)
            {
                FM_LOG_ERR(D_IOCTL,"ERROR, FM chip is OFF\n");
                return -EFAULT;
            }
            if(copy_from_user(&parm, (void*)arg, sizeof(struct fm_softmute_tune_cqi_t)))
            {
                FM_LOG_ERR(D_IOCTL|D_ALL,"copy_from_user error\n");
                return -EFAULT;
            }
            ...
            if(copy_to_user((void*)arg, &parm, sizeof(struct fm_softmute_tune_cqi_t)))
            {
                FM_LOG_ERR(D_IOCTL|D_ALL,"copy_to_user error\n");
                return -EFAULT;
            }
		...
		{
			fm_s32 tmp;
            FM_LOG_NTC(D_IOCTL,"......FM_IOCTL_IS_DESE_CHAN......\n");
    ...
    case FM_IOCTL_TUNE_NEW: {
        ...
        FM_LOG_DBG(D_IOCTL, "......FM_IOCTL_TUNE_NEW......\n");
        ...
        if (copy_from_user(&tune_tmp, (void*)arg, sizeof(struct fm_tune_t))) {
            FM_LOG_ERR(D_IOCTL | D_ALL, "tune new copy_from_user error\n");
            return -EFAULT;
        }
        ...
        if (copy_to_user((void*)arg, &tune_tmp, sizeof(struct fm_tune_t))) {
            FM_LOG_ERR(D_IOCTL | D_ALL, "tune new copy_to_user error\n");
            return -EFAULT;
        }
    ...
    case FM_IOCTL_SEEK_NEW: {
        ...
        FM_LOG_DBG(D_IOCTL, "......FM_IOCTL_SEEK_NEW......\n");
        ...
        if (copy_from_user(&seek_tmp, (void*)arg, sizeof(struct fm_seek_t))) {
            FM_LOG_ERR(D_IOCTL | D_ALL, "seek new copy_from_user error\n");
            return -EFAULT;
        }
        ...
        if (copy_to_user((void*)arg, &seek_tmp, sizeof(struct fm_seek_t))) {
            FM_LOG_ERR(D_IOCTL | D_ALL, "seek new copy_to_user error\n");
            return -EFAULT;
        }
    ...
    case FM_IOCTL_SCAN_NEW: {
        ...
        FM_LOG_DBG(D_IOCTL, "......FM_IOCTL_SCAN_NEW......\n");
        ...
        switch (tmp.cmd) {
            case FM_SCAN_CMD_START: 
                if ((tmp.upper > 10800) || (tmp.lower < 7600) || (tmp.space < 5) || (tmp.space > 20)) {
                    FM_LOG_ERR(D_IOCTL | D_ALL, "scan para error\n");
                    return -EFAULT;
                }
                parm.cmd = tmp.cmd;
            ...
            case FM_SCAN_CMD_GET_CH_RSSI:
                if (parm.sr_size && parm.sr.ch_rssi_buf) {
                    if (copy_to_user(tmp.sr.ch_rssi_buf, parm.sr.ch_rssi_buf, parm.num * sizeof(struct fm_ch_rssi))) {
                        FM_LOG_ERR(D_IOCTL | D_ALL, "scan copy_to_user err\n");
                        return -EFAULT;
                    }
                }
                break;
...
//              return -EPERM;
    ...
    case FM_IOCTL_RDS_BC_RST: {
        FM_LOG_DBG(D_IOCTL, "......FM_IOCTL_RDS_BC_RST......\n");
        ...
        ret = MT6620_RDS_Reset_Block_Counter();
...
//                return -EPERM;
        ...
        case FM_IOCTL_GET_I2S_INFO: {
            FM_LOG_DBG(D_IOCTL,"......FM_IOCTL_GET_I2S_INFO......\n");

            if (!arg) {
                FM_LOG_ERR(D_IOCTL|D_ALL,"get i2s info, user space invalid NULL pointer\n");
                return -EFAULT;
            }
            
            if (copy_to_user((void*)arg, &i2s_info, sizeof(struct fm_i2s_info))) {
                FM_LOG_ERR(D_IOCTL|D_ALL,"get i2s info, copy_to_user error\n");
                return -EFAULT;
            }
...
static int fm_init()
{
	...
	g_fm_proc = create_proc_entry(FM_PROC_FILE, 0664, NULL);
	if (g_fm_proc == NULL){
	...
	g_fm_config.proc = create_proc_entry(FM_PROC_CONFIG, 0660, NULL);
...
static int  fm_soft_mute_tune(struct fm *fm, struct fm_softmute_tune_cqi_t *parm)
{
	...
	pkt_size = mt6620_soft_mute_tune(tx_buf, TX_BUF_SIZE, parm->freq,1,1);
	ret = fm_send_wait_timeout(tx_buf, pkt_size,FLAG_SM_TUNE, 1, SM_TUNE_TIMEOUT);
	if(!ret)
	{
		FM_LOG_NTC(D_G0, "smt cqi size %d\n", soft_mute_tune_CQI[0]);
		//parm = (struct fm_softmute_tune_cqi_t *)&soft_mute_tune_CQI[2];
		parm->freq = (soft_mute_tune_CQI[3]<<8)|soft_mute_tune_CQI[2];
		parm->RSSI = (soft_mute_tune_CQI[5]<<8)|soft_mute_tune_CQI[4];
		parm->PAMD = (soft_mute_tune_CQI[7]<<8)|soft_mute_tune_CQI[6];
		parm->MR = (soft_mute_tune_CQI[9]<<8)|soft_mute_tune_CQI[8];
		parm->ATDC = (soft_mute_tune_CQI[11]<<8)|soft_mute_tune_CQI[10];
		parm->PRX = (soft_mute_tune_CQI[13]<<8)|soft_mute_tune_CQI[12];
		parm->SMG = (soft_mute_tune_CQI[15]<<8)|soft_mute_tune_CQI[14];
	
		FM_LOG_NTC(D_G0, "freq %d, 0x%04x, 0x%04x, 0x%04x, 0x%04x, 0x%04x, 0x%04x\n", 
			parm->freq, 
			parm->RSSI,
			parm->PAMD,
			parm->MR,
	...
	{
		FM_LOG_WAR(D_CMD,"mt6620_soft_mute_tune Failed\n");
	}
out:
	up(&fm_cmd_mutex);
	FM_LOG_INF(D_CMD,"+fm_cmd_mutex\n");
...
out1:
	FM_LOG_NTC(D_CMD,"-%s():[ret=%d]\n", __func__, ret);
	return ret;
}
int fm_is_dese_chan(struct fm *pfm, fm_u16 freq)
{
    if ((priv.state == INITED) && (priv.priv_tbl.is_dese_chan)) 
    {
        return priv.priv_tbl.is_dese_chan(freq);
    }
...
static int fm_scan_new(struct fm *fm, struct fm_scan_t *para)
{
    ...
    uint16_t scan_tbl[MT6620_SCANTBL_SIZE]; //need no less than the chip
    uint16_t tbl_size = MT6620_SCANTBL_SIZE;
    uint16_t tmp_freq = 0;
    int chl_cnt;
    int ch_offset, step, tmp_val;
    int space_idx = MT6620_FM_SPACE_100K;

    // caculate channel number, get segment count
    cnt = (para->upper - para->lower) / para->space + 1; //Eg, (10800 - 8750) / 5 = 411
    seg = (cnt / SCAN_SEG_LEN) + ((cnt % SCAN_SEG_LEN) ? 1 : 0); //Eg, (411 / 200) + ((411 % 200) ? 1 : 0) = 2 + 1 = 3

    FM_LOG_NTC(D_MAIN, "total ch %d, seg %d\n", cnt, seg);
    
    // alloc memory
    tmp = cnt * sizeof(struct fm_ch_rssi*);
    if (para->sr_size < tmp) {
        if (para->sr.ch_rssi_buf) {
            kfree(para->sr.ch_rssi_buf);
            para->sr.ch_rssi_buf = NULL;
        }
        para->sr_size = tmp;                
    }
    
    if (!para->sr.ch_rssi_buf) {
        para->sr.ch_rssi_buf = (struct fm_ch_rssi*)vmalloc(para->sr_size);
        if (!para->sr.ch_rssi_buf) {
            FM_LOG_ERR(D_MAIN, "scan alloc mem failed\n");
            para->sr_size = 0;
            return -2;
        }
    }

    // remap space to space_index
    if (para->space == 5) {
        space_idx = MT6620_FM_SPACE_50K;
    } else if (para->space == 10) {
        space_idx = MT6620_FM_SPACE_100K;
    } else if (para->space == 20) {
        space_idx = MT6620_FM_SPACE_200K;
    }
    
    // do scan
    chl_cnt = 0;
    for (i = 0; (i < seg) && (0 == stop_scan_flag); i++) {
        start_freq = para->lower + SCAN_SEG_LEN * para->space * i;
        end_freq = para->lower + SCAN_SEG_LEN * para->space * (i + 1) - para->space;
        end_freq = (end_freq > para->upper) ? para->upper : end_freq;
        
        FM_LOG_NTC(D_MAIN, "seg %d, start %d, end %d\n", i, start_freq, end_freq);
        ret = MT6620_Scan(start_freq, end_freq, &tmp_freq, scan_tbl, &tbl_size, MT6620_FM_SCAN_UP, space_idx);
        if (ret < 0) {
            return ret;
        }

        // get channel count
        for (ch_offset = 0; ch_offset < MT6620_SCANTBL_SIZE; ch_offset++) {
		    if (scan_tbl[ch_offset] == 0)
			    continue;
		    for (step = 0; step < 16; step++) {
			    if (scan_tbl[ch_offset] & (1 << step)) {
                    tmp_val =  start_freq + (ch_offset * 16 + step) * para->space;
                    if (tmp_val <= end_freq) {
                        // record valid  result channel
                        FM_LOG_NTC(D_MAIN, "freq %d\n", tmp_val);
                        para->sr.ch_rssi_buf[chl_cnt].freq = tmp_val;
                        chl_cnt++; 
                    }
			    }
		    }
    ...
    // get cqi info
    for (i = 0; (i < chl_cnt) && (0 == stop_scan_flag); i++) {
        ret = MT6620_Fast_SetFreq(para->sr.ch_rssi_buf[i].freq);
        if (ret)
            return ret;
        ...
        para->sr.ch_rssi_buf[i].rssi = total / 2;
        FM_LOG_NTC(D_MAIN, "freq %d, rssi %d\n", para->sr.ch_rssi_buf[i].freq, para->sr.ch_rssi_buf[i].rssi);
    }

    para->num = chl_cnt;
    stop_scan_flag = 0; //reset flag
    return ret;
}


static int fm_seek_new(struct fm *fm, struct fm_seek_t *parm)
{
    int ret = 0;
    int space_idx = MT6620_FM_SPACE_100K;

    if (parm->freq < parm->lower || parm->freq > parm->upper) {
        FM_LOG_ERR(D_MAIN, "seek start freq:%d out of range\n", parm->freq);
        return -EPERM;
    }

    ret = MT6620_Fast_SetFreq(parm->freq);
    if (ret < 0) {
        return ret;
    }
    ...
    ret = MT6620_Seek(parm->lower, parm->upper, &(parm->freq), parm->dir, space_idx);
    if (ret < 0) {
        return ret;
    }
...
static int fm_tune_new(struct fm *fm, struct fm_tune_t *parm)
{
    int ret = 0;
    int hl_side = -1;
    int freq_avoid = -1;
...
int fm_priv_register(struct fm_priv *pri, struct fm_op *op)
{
    ...
    priv.priv_tbl.is_dese_chan = pri->priv_tbl.is_dese_chan;
    priv.state = INITED;
    priv.data = NULL;
