...
static u32 DDCM_TrigMode(u32 u4Mode)
{
  ...
  while(IS_SIF_BIT(DDC_DDCMCTL1,DDCM_TRI)){udelay(1);}

  return (0);
}

static u8 _DDCMRead(u8 ucCurAddrMode, u32 u4ClkDiv, u8 ucDev, u32 u4Addr, SIF_BIT_T ucAddrType, u8 *pucValue, u32 u4Count)
{
	u32 u4Ack;
...
static u8 _DDCMWrite(u8 ucCurAddrMode, u32 u4ClkDiv, u8 ucDev, u32 u4Addr, SIF_BIT_T ucAddrType, const u8 *pucValue, u32 u4Count)
{
	u32 u4Ack;
...
u32 DDCM_RanAddr_Write(u32 u4ClkDiv, u8 ucDev, u32 u4Addr, SIF_BIT_T ucAddrType, const u8 *pucValue, u32 u4Count)
{
	u32 	u4WriteCount1;
	u8 	ucReturnVaule;
...
u8 DDCM_RanAddr_Read(u32 u4ClkDiv, u8 ucDev, u32 u4Addr, SIF_BIT_T ucAddrType, u8 *pucValue, u32 u4Count)
{
	u32 u4ReadCount;
	u8 	ucReturnVaule;
...
{
  MT8193_DDC_FUNC();
  if(DDCM_RanAddr_Read(SIF1_CLOK, (u8)bDevice, (u32)bData_Addr, SIF_8_BIT, (u8 *)prData, (u32)bDataCount)== bDataCount)
    return TRUE;	
  else  
	return FALSE;	
}	
...
{
	MT8193_DDC_FUNC();
	if((DDCM_RanAddr_Write(SIF1_CLOK,  (u8)bDevice, (u32) bData_Addr, SIF_8_BIT,  (u8 *)prData, (u32)bDataCount))== bDataCount)
	  return TRUE;	  
	else  
	  return FALSE;
}
