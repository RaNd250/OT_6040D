...
#endif
#define TOPOFF_CHG_VOLTAGE      4200 
...
#define CHG_MIN_BYD_BATT_ID  700
#define CHG_MAX_BYD_BATT_ID  900
#define CHG_MIN_SCUD_BATT_ID  1250
#define CHG_MAX_SCUD_BATT_ID  1350
int byd_sucd_batt_id;// 1 BYD 2 SUCD
...
#if defined(DIABLOX_CHARGING_TEMPERATURE_PLOICY) 
if(g_bat_temperature_pre < 38)
    {
#endif
 ...
 else if (g_bat_temperature_pre < 41)
 {
 reg_set_value=0x40;
 }
 else if (g_bat_temperature_pre < 43)
 {
 reg_set_value=0x30;
 }
 else
 ...
 // add temperature 38,40,43 degree charging policy
#endif
    if(g_low_power_ready == 1)
        reg_set_value += 0x09;
    else
...
{    
	...
	if((BMT_status.TOPOFF_charging_time > 60*60) && (BMT_status.bat_charging_state == CHR_TOP_OFF))
        {
	...
	//fan5405_config_interface_liao(0x04,0xeb);
