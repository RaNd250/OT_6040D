...
{
    if (!strncmp(buf, "MM", strlen("MM1"))) {
        bm_master_evt = BM_MASTER_MM1;
    }else if (!strncmp(buf, "APMCU", strlen("APMCU"))) {
        bm_master_evt = BM_MASTER_AP_MCU;
    }else if (!strncmp(buf, "MD_ARM9", strlen("MD_ARM9"))) {
        bm_master_evt = BM_MASTER_MD_ARM9;
        ...
        bm_master_evt = BM_MASTER_MD_ARM9 | BM_MASTER_MD_MCU | BM_MASTER_2G_3G_MDDMA;   
    }else if (!strncmp(buf, "MM0_PERI", strlen("MM0_PERI"))) {
        bm_master_evt = BM_MASTER_MM0_PERI;
...
//    *(volatile unsigned int *)0xF702A000 |= 0x1000;
    ...
    MCI_Event_Set(0x2, 0x4); /*0x2: AC_R snoop transaction counts; 0x4: AC_R snoop hit*/
