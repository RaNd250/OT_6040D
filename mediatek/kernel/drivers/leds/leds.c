...
static unsigned int notify_led_type_enable = 1;
/*****************PWM *************************************************/
...
static int led_set_pwm(int pwm_num, struct nled_setting* led)
{
	...
	LEDS_DEBUG("[LED]led_set_pwm: mode=%d,pwm_no=%d\n", led->nled_mode, pwm_num); 
        int time_index = 0;
		printk("[LED]led_set_pwm:MT6320_PAD_PWM1=%d,MT6320_PAD_PWM3=%d,pwm_num=%d------1\n",MT6320_PAD_PWM1,MT6320_PAD_PWM3,pwm_num);
        if (pwm_num >= MT6320_PAD_PWM1 && pwm_num <= MT6320_PAD_PWM3){
			printk("[LED]led_set_pwm:MT6320_PAD_PWM1=%d,MT6320_PAD_PWM3=%d,pwm_num=%d--------2\n",MT6320_PAD_PWM1,MT6320_PAD_PWM3,pwm_num);
            pwm_setting.pwm_no = pwm_num - 3;
            pwm_setting.pmic_pad = 1;
        } else {
			...
			}else
			{
				if(level == 0)
				{
				       if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(GPIO28,GPIO_OUT_ZERO);//for power saving
						}
				...
				}else
				{
				      if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(GPIO28,GPIO_OUT_ONE);//for power saving
							if(notify_led_type_enable == 0)
				...
				if(level == 0){
				if((!strcmp(cust->name,"red")) && notify_led_type_enable == 0){
			...
			if(notify_led == 1)
				{
					...
					if(notify_led_type_enable == 0){
						...
						led_set_pwm(PWM7, &led_tmp_seting);
...
//static unsigned int notify_led_type_enable;
static ssize_t show_notify_led_type(struct device *dev,struct device_attribute *attr,char *buf)
{
	unsigned int notify_led_type = notify_led_type_enable;
	return sprintf(buf,"%u\n" ,notify_led_type);
	return 0;

}

static DEVICE_ATTR(notify_led_type,0664, show_notify_led_type,NULL);
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	//wake_lock_init(&led_bright_lock, WAKE_LOCK_SUSPEND, "led_bright_lock");
	
#if RAV4_LED
	 mt_set_gpio_mode(GPIO35, GPIO_MODE_00);
	 mt_set_gpio_dir(GPIO35, GPIO_DIR_IN);
	 mt_set_gpio_pull_enable(GPIO35, 0);
#endif 
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
	    ...
	    rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_pwm_register);
	     ...
	     if(strcmp(g_leds_data[i]->cdev.name,"red")==0)
	     {
		  ...
		  if(rc)
		  {
			LEDS_DEBUG("[delay_on] device create file duty fail!\n");	
		  }
		  rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_delay_off);
		  if(rc)
		  {
		       LEDS_DEBUG("[delay_off] device create file duty fail!\n");	
		  }
		  rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_notify_led_type);
		  if(rc)
		  {
		       LEDS_DEBUG("[notify_led_type] device create file duty fail!\n");	
		  }
#if RAV4_LED
		  if (mt_get_gpio_in(GPIO35)==1)
		  {    // g_leds_data[i]->cust.mode = PWM1;
		  	  g_leds_data[i]->cust.data = PWM1;
			  notify_led_type_enable = 0;
			  mt_set_gpio_mode(GPIO73, GPIO_MODE_01);
			  mt_set_gpio_mode(GPIO28, GPIO_MODE_00);
			  mt_set_gpio_dir(GPIO28, GPIO_DIR_OUT);
			  mt_set_gpio_out(GPIO28, GPIO_OUT_ZERO);
		...
		{
#endif	  
		        g_leds_data[i]->cust.data = PWM7;
			 notify_led_type_enable = 1;
