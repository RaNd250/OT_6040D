...
/*=======================================================================
  ...
  * =>Add imx135 NR set and temperature get interface for test
  *=======================================================================*/
#define CAM_SYSFS_SUPPORT
#ifdef CAM_SYSFS_SUPPORT
//#define IMX135MIPI_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, IMX135MIPI_WRITE_ID)
//extern void IMX135MIPISetNR(int cnrVal, int arnrVal);
//extern int IMX135GetTemperature(void);
int temperature, cnrval, arnrval;

static ssize_t CAM_value_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char *s = buf;
    int ret;
    U8 val[4];
    s += sprintf(s, "imx135 CNR and ARNR value: %d %d\n", cnrval, arnrval);
    s += sprintf(s, "imx135 current temperature: %d\n", temperature);
    return (s - buf);
}


static ssize_t CAM_value_store(struct kobject *kobj, struct kobj_attribute *attr,
               const char *buf, size_t n)
{
    ...
    kal_uint8 i;
    int ret;
    sscanf(buf, "%s %d %s %d", (char *)&cmd_str, &val, (char *)&cmd_str1, &val1);

    if(strcmp(cmd_str, "camlog") == 0)
    {
        for ( i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ )
        {
            if (g_pInvokeSensorFunc[i])
            {
                ret = g_pInvokeSensorFunc[i]->SensorFeatureControl(SENSOR_FEATURE_LOG_SWITCH,&val,1);
            }
        }
    }
/*
    if((strcmp(cmd_str, "cnr") == 0)&&(strcmp(cmd_str1, "arnr") == 0))
    {
        cnrval = val;
        arnrval = val1;
        IMX135MIPISetNR(cnrval, arnrval);
    }

    if(strcmp(cmd_str, "camtemp") == 0)
    {
        temperature = IMX135GetTemperature();
    }
*/
    return n;
}
...
static DEVICE_ATTR(CAM_DEBUG, 0644,  CAM_value_show, CAM_value_store);  //0666->0664 

static struct attribute *CAM_sysfs_attrs[] = {
    &dev_attr_CAM_DEBUG.attr,
    NULL,
};
...
//add sysfs
struct kobject *CAM_ctrl_kobj;
static int CAM_sysfs_init(void)
{ 
    CAM_ctrl_kobj = kobject_create_and_add("CAM", NULL);
    if (!CAM_ctrl_kobj)
        return -ENOMEM;

    return sysfs_create_group(CAM_ctrl_kobj, &CAM_attr_group);
}
//remove sysfs
static void CAM_sysfs_exit(void)
{
    sysfs_remove_group(CAM_ctrl_kobj, &CAM_attr_group);

    kobject_put(CAM_ctrl_kobj);
}
#endif
//zhfan end
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    CAM_sysfs_init(); //zhfan for camera sys fs pr458701
