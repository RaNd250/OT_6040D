...
#define JRD_FIX_PR432080 
#ifdef JRD_FIX_PR432080
static unsigned int jrd_check_type_cnt = 0;//Record the accdet module interruption time.
#endif
...
#define PIN_ADC_CHANNEL 5
	...
	case HOOK_SWITCH:
            ...
            {
#if 1//Added by jrd.lipeng for headset type detecting error
                /* Sometimes a headset will be detected as a headphone when insert the headset slowly, which
                  * cause an AB=0 accdet interrupt. After inserting the whole jack, an AB=1 interrupt will come, 
                  * then a 'HEADSET_MIC' cable_type is reported. However, framework will ignore this change.
                  * So I report a plug out event firstly, then this late correct headset type detecting will be accepted 
                  * by framework.*/
#ifdef JRD_FIX_PR432080
                ACCDET_DEBUG("[Accdet] jrd jrd_check_type_cnt: %u", jrd_check_type_cnt);//lipeng debug
#endif
                if(cable_type == HEADSET_NO_MIC && jrd_check_type_cnt < 4) {
                    ACCDET_DEBUG("[Accdet] I think a headset is detected as a headphone. So correct it.");
                    headset_plug_out();
                }
#endif
				mutex_lock(&accdet_eint_irq_sync_mutex);
